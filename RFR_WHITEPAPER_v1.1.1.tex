...
\author{
  ...
  Version 1.1.1\\[0.25em]
  DOI (v1.1.1): \href{https://doi.org/10.5281/zenodo.21403659}{10.5281/zenodo.21403659}\\
  Concept DOI: \href{https://doi.org/10.5281/zenodo.20329104}{10.5281/zenodo.20329104}
}
\date{17 July 2026}
...
\paragraph{Internal closure (added in v1.1.1).}
The two checks above constrain only \emph{outside} candidates. They do not
constrain causal chains that run entirely \emph{within} the band. For each
$u\in\band$ and outgoing edge $(u,v)$ with $v\in\band$, the safety check
must also reject the batch unless:
\[
  \hat d(v) \le \hat d(u) + w(u,v).
\]
If any within-band edge can still lower its target, the band members are
causally linked: settling them together can finalise a member at a stale
label even though the improving chain lies entirely inside the batch. Such a
band is unsafe and must be split or resolved locally. Versions prior to
1.1.1 omitted this condition; Section~\ref{sec:erratum} documents the
resulting counterexample and its discovery by a machine-checked
implementation of the safety relation.
...
\textbf{Lemma.} If a band $\band$ passes the three safety checks in Section~\ref{sec:safety} (outside precedence, outside candidate creation, and internal closure), then settling the members of $\band$ in nondecreasing tentative-distance order preserves the label-setting invariant.

\textbf{Sketch.} The first safety check ensures that no known outside frontier vertex has a smaller tentative distance than any member of $\band$. The second safety check ensures that relaxing any band member cannot create an outside candidate whose distance is smaller than the maximum remaining distance within the band. The third check (internal closure) ensures that no within-band edge can improve any band member: the current labels are already a fixed point of relaxation restricted to the band. Together these imply that no candidate --- outside or inside --- can lower any member's label after finalisation, so the tentative labels equal the true distances and each settled vertex has the globally minimal necessary label at its settlement point. The v1.1.0 sketch argued only the first two conditions and incorrectly concluded exactness; internal closure is necessary, as the counterexample in Section~\ref{sec:erratum} shows.
...
\section{Erratum (v1.1.1): Internal Closure}
\label{sec:erratum}

Versions 1.0.0--1.1.0 of this paper stated the batch safety relation with two
conditions (outside precedence and outside candidate creation) and the
implementation enforced exactly those two. Both the lemma sketch and the code
omitted a necessary third condition: \emph{internal closure} of the band under
relaxation.

The omission admits incorrect results. Counterexample (edge list
$S\!\to\!A$:1, $S\!\to\!B$:4, $S\!\to\!C$:9, $A\!\to\!C$:1, $C\!\to\!B$:1,
$B\!\to\!D$:10; band width 10): all vertices fall into one band that passes
both original checks. The improving chain $A$ lowers $C$ ($9\to2$), $C$ lowers
$B$ ($4\to3$) runs entirely within the band, so $B$ is finalised at its stale
label 4 and its outgoing edge never refires: the solver returns $d(D)=14$
against the correct $d(D)=13$.

The counterexample was found by a machine-checked implementation of the
safety relation (Lean), constructed as an instantiation of the Cogenesis
framework's requirement that safety conditions be stated as checkable
relations. The checker enforced internal closure as part of
the relation; the divergence between checker and implementation localised the
omitted condition. The fix (v1.1.1, and the accompanying code release) adds
the internal-closure check to the safety predicate; ambiguous bands split or
fall back to local exact processing, as before.

Scope of impact: the error affects the general-graph solver only, and only at
band widths exceeding the minimum edge weight. The invariant-band corollary
and field solver (Sections~\ref{sec:invariantcorollary}
and~\ref{sec:invariantresults}) are unaffected by construction: with band
width at most the minimum step cost, no within-band edge can exist, so
internal closure holds vacuously. All v1.1.0 benchmark results stand. The
regression suite now pins the counterexample across band widths, and a
randomized differential test against \Dijkstra{} (2{,}400 runs, 400 graphs)
passes with the corrected predicate.
